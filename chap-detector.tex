...
%This is done using a deterministic annealing algorithm. For a given cluster of 
%tracks, the position of the associated vertex is then estimated by means of an 
...
% also jet section 6 in STFC notes
% cone algorithm: start with seed and cluster everything within cone DR, then 
%use this as the new seed and repeat until direction of cone is stable. 
%Problems: how to define seed, can have overlaps (particle matched to more than 
%one seed), not infrared safe (soft emission between two particles can lead to 
%one big jet rather than two individual ones). Most popular cone algorithm is 
%SISCone which avoids use of seeds and treats overlapping jets carefully and is 
%infrared safe
...
% fragmentation is the showering/breaking apart of quarks (string), 
%hadronisation is the subsequent coming together of quarks to form 
%energetically favourable colour-neutral states
...
% distance measure chosen is invariant under longitudinal boosts (along beam 
%direction)
...
% "combining" particles means summing their four-momenta ("E-scheme")
...
%%circular jets.
% p=-1 takes hard particles (which dominate min function) and clusters soft 
%stuff around it, whereas p=1 clusters soft stuff (which dominate min function) 
%with itself preferentially. Sandro: the algorithm will tend to cluster soft 
%particles with the hard ones instead of along themselves.
...
%cone but this computation is complicated by the detector geometry. An eective 
...
% feynman diagram -> matrix element (square gives prob) -> integrate over phase 
%space to get cross section, or don't integrate completely to get differential 
%xs (function of momentum and direction, which you can then MC-sample)
...
%Baber: In ation, a simulation of the splitting of the partons from the 
