...
total background) using dedicated variables. % the variables described in 
%Sec.~\ref{sec:analysis-qcdrejection}. 
...
calibrations in data. %\textbf{This is described in Sec.~X}. 
...
In order to maximise the sensitivity to a wide range of BSM physics %SUSY %and 
%%DM 
scenarios, 
...
b-tagged jets. %as detailed in Sec.~\ref{sec:analysis-binning}. 
...
employed in the background estimation.%s as described in 
%Sec.~\ref{sec:analysis-estimation-ewk}.
%inclusiveness
%The ways in which a WIMP/BSM signature can manifest are numerous

Similar searches for supersymmetry have previously been performed %in Runs 1 
%and 2 of the 
...
collected. In addition, the interpretation in the context of %s in dark matter 
%and 
long-lived particles % described in Chap.~\ref{chap:results} 
...
% (see Sec.~\ref{sec:detector-jets}). 
%Charged particles that originate from pileup vertices are not included. 
...
\begin{table}[ht!]
%: Table: jet ID requirements (have 3 columns for each eta region, merge 
...
$\sim$98\%. 
%The contribution to the muon energy from pileup tracks is subtracted using the 
...
%don't want non-prompt (produced from the in-flight decays of hadrons, taus or 
%heavy quarks) and to reject fakes caused by the punch through of hadronic
...
%The effective area pileup correction is applied. 
...
%\textbf{because of [fixme] QCD and trigger efficiency???}}
...
$\sim$71\%. 
%The effects of pileup are mitigated using the effective area corrections.
...
%Effective area pileup corrections and a
...
%The \scalht and \MHT variables are defined, respectively, as the scalar sum 
%and the magnitude of the negative vector sum of the transverse energy of all 
%jets in the event satisfying \pt~$>40$~GeV and \etaabs~$<2.4$.
...
%The missing transverse energy \met is computed as the magnitude of the 
%negative vector sum of the transverse energy of all PF particles in the event. 
%This variable is used in the definition of the \mt and \mhtmet variables as 
%described in 
...
energy that is typical of the SUSY %and DM 
...
%Primary vertex selection? MC and AE have it in MET filter section. See 
%vertex skimmer in heppy - "good vertex".
% Dunne: events are required to have a real vertex, which has ndof > 4 and a 
%maximum displacement in the z-direction (xy-plane) direction from the centre 
%of the detector of 24 cm (2 cm).
%• a minimum number of degrees of freedom: ndof > 4
%• the collision to occur with |z| < 24 cm such that the primary vertex is near 
%the interaction point in the longitudinal direction
%• the collision to occur within a radial distance of |dxy| < 2 cm from the 
%beamline
At least one good quality vertex is required to be reconstructed in the event. 
...
%If have time make diagrams illustrating alphat and bdphi.
...
%[ already defined in MHT definition?]
...
% hatches are stat uncs; last bin is overflow
%[: actually, presel left full sel right]}
\label{fig:alphat}
\end{center}
\end{figure}
...
% [FIXME: remove ratio plot and fix axis label? Update to 36fb]}
\label{fig:mhtmet}
\end{center}
\end{figure}
...
%This and severe mismeasurements below threshold are the two dominantmechanisms 
...
% little signal contam even for signal models with tops and bottoms because of 
%MT and Mll cuts
...
%[ not sure if this last sentence is needed or is already 
%obvious/mentioned previously]. 
...
%The lower bound on the \bdphi 
%sideband and upper bound on the \mhtmet sideband are imposed because [FIXME] 
%modelling not right or what? ah trigger? mhtmet trigger loses eff when met 
%low. 
...
%\textbf{also two MHT sidebands for sideband corrs. 
%also mhtmet and bdphi 
...
%targeting many models of dark matter production, which are also identified to 
%a large extent via additional radiation recoiling against the invisible system.
%\textbf{ LL SUSY: monojet. SL (short-lived) DM: multijet.}
...
$\scalht>1200$~GeV. 
%For convenience, % easy of presentation/interpretation
...
is not too large.
%\textbf{TODO: What's the stat requirement for HT bins? paper: merged to 
%satisfy 
%a threshold on the minimum number of events in the corresponding bins of the 
%CRs. this was eyeballed?} 
...
%\textbf{an extrapolation in nb is performed for the region nb geq 1. The 
...
%(in particular DM and compressed SUSY models), 
...
to ensure each bin is well populated by simulated events. %(see 
%Sec.~\ref{sec:analysis-estimation-mht}). 
%TODO such that there are at least 500 unweighted simulation events in each 
%(\njet, \scalht, \mht) bin [FIXME] to ensure well populated mht/nb templates. 
For events with $\njet=1$, for which 
...
which %, due to the `curse of dimensionality', 
...
%%% I took first and last run in json, L1 prescales are in WBM under "Prescale 
%Sets" (I also found the HLT prescales)
...
% thresholds kept relatively low thanks to L1 upgrade.
...
%Particle Flow
%algorithm which clusters all PF particles. These `fake' jets are then removed 
%from the event if they are close to an isolated lepton, in a process known as 
%\textit{cross-cleaning}. 
...
\begin{figure}%[t]
\subfloat[$200 < \scalht < 
400$~GeV]{\includegraphics[width=0.29\textwidth]{figs/analysis/Trigger_Ele_HLT_AlphaTHT800MonoAll_MoM_all_200to400_mht}}
~~
\subfloat[$200 < \scalht < 
400$~GeV]{\includegraphics[width=0.29\textwidth]{figs/analysis/Trigger_Muon_HLT_AlphaTHT800MonoAll_MoM_all_200to400_mht}}
\\
\subfloat[$400 < \scalht < 
600$~GeV]{\includegraphics[width=0.29\textwidth]{figs/analysis/Trigger_Ele_HLT_AlphaTHT800MonoAll_MoM_all_400to600_mht}}
~~
\subfloat[$400 < \scalht < 
600$~GeV]{\includegraphics[width=0.29\textwidth]{figs/analysis/Trigger_Muon_HLT_AlphaTHT800MonoAll_MoM_all_400to600_mht}}
\\
\subfloat[$600 < \scalht < 
900$~GeV]{\includegraphics[width=0.29\textwidth]{figs/analysis/Trigger_Ele_HLT_AlphaTHT800MonoAll_MoM_all_600to900_mht}}
~~
\subfloat[$600 < \scalht < 
900$~GeV]{\includegraphics[width=0.29\textwidth]{figs/analysis/Trigger_Muon_HLT_AlphaTHT800MonoAll_MoM_all_600to900_mht}}
\\
\subfloat[$\scalht > 900$~GeV]      
{\includegraphics[width=0.29\textwidth]{figs/analysis/Trigger_Ele_HLT_AlphaTHT800MonoAll_MoM_all_900to999999_mht}}
~~
\subfloat[$\scalht > 900$~GeV]      
{\includegraphics[width=0.29\textwidth]{figs/analysis/Trigger_Muon_HLT_AlphaTHT800MonoAll_MoM_all_900to999999_mht}}
\\
...
%\textbf{lumi uncertainty and reference (or just leave for corrections 
%section).}
...
%\textbf{no need to mention pythia hadronisation and geant here?}
...
% approximately 50-50 charged-uncharged (can also change charge as it traverses)
%In this  model, R-hadrons are treated as supersymmetric particles surrounded 
%by a cloud of loosely bounded quarks or gluons.
% see atlas disp verts p5 for r-hadron and matter interaction description and 
%references
...
% see stopped LLPs and hscp 
%Studies 341 with alternative values for parameters that influence the 
%hadronization of the gluino, such as 342 Pgeg = 50\%, indicate a minimal 
%influence on the event topology and kinematical variables for 343 the models 
%considered in this paper.
%default is no MI, but also check behaviour of MI.
%check consistent with paragraph in results.
%RHADRONS (see paper), baryon and meson states, gluino ball - 
...
% the tag would've added "1" to the efficiency
...
ratio.
% difference tnp MINUS true = 1-2% (but effs are high so difference is almost 
%%the same as ratio: effTnp-effTrue/effTrue ~approx effTnp-effTrue)
\begin{comment}
bias. WHY NOT COUNT TAG AS PASSING PROBE? then would be biased higher because 
not counting events where neither muon is tag (you're only counting events in 
which there is at least a tag so you'll always get one "1" in eff)
...
% if second muon out of acceptance, don't care, simulation doesn't mismodel 
%detector size
% if second muon doesn't pass probe, don't care, because we are measuring eff 
%*conditional* on previous step. This would only affect the first step (ID 
%conditional on track) but just say we do actually have tracking eff 
%corrections 
%(derived separately - check how it's done) and eff is close to 100% anyway
% if neither is tag, you simply don't make a measurement, not necessarily 
%biased as depends on whether they are passing or failing probes

% I tag, then check if other muon passes or not (unconditional), fine
% If other muon also tag, I tag it, check if other muon passes (also uncond.)
% If neither is tag, I check if muon passes or not conditional on not tag 
%- we're not counting these events (see my diagram)

% can remove bias by only tagging on positive charge muon, and probing the 
%negative charge muon
...
where the products are over all corresponding probes $i$, and the 
...
99\%$.
%The correction factors are then used to weight the simulated events in the 
...
% or maybe it's fine, point is you multiply all epsilons in equation by SF

The efficiencies and correction factors are then propagated to the simulated 
...
%%% ignoring tau - not relevant, we don't select or veto them
...
%and answers from Phil or whatevs. Describe values/trends of TFs.}
%ONLY IF YOU UNDERSTAND THIS.
%\begin{figure}[t]
%\subfloat{\includegraphics[width=0.8\textwidth]{figs/analysis/transferfactors/tf_mu_Ttw_2d_nominalUp}}\\
%\subfloat{\includegraphics[width=0.8\textwidth]{figs/analysis/transferfactors/tf_mumu_Zinv_2d_nominalUp}}\\
%\caption{transfer factors (top) and (bottom) }
%\end{figure}
...
Although the \mmj control region is binned more inclusively in the \nb 
...
%instead of predicted from CRs via transfer factors
%Take templates because don't want to bin CRs too finely (lose statistical 
%power - curse of dimensionality).
...
%Validation: Check data MC ratio is flat in CRs.
...
%contained within 3 sigma, gaussian distributed
%Show example fit.
%Show distribution of pulls(t/z values)/pvalues. check mean is consistent with 
%0.
...
\subfloat[$\njet=4, \nb=2, 
\scalht=$~600-750~GeV 
(\mj)]{\includegraphics[width=0.49\textwidth]{figs/analysis/mht_eq2b_eq4j_ht_600_800_SingleMu_Graph}}
\subfloat[$\njet=2, \nb=1, 
\scalht=$~400-500~GeV 
(\mmj)]{\includegraphics[width=0.49\textwidth]{figs/analysis/mht_eq1b_eq2j_ht_400_500_SinglePhoton_Graph}}
...
\begin{figure}
\centering
\subfloat[\mj]{\includegraphics[width=0.49\textwidth]{figs/analysis/mht_pulls_mu}}
\subfloat[\mmj]{\includegraphics[width=0.49\textwidth]{figs/analysis/mht_pulls_mumu}}
...
The QCD background estimation is performed in a similar way as the 
...
%Plot of estimated yields and qcd/ewk ratio per bin.
\begin{figure}[!h]
\centering
\subfloat{\includegraphics[width=0.7\textwidth]{figs/analysis/meanPredQcdYields}}
 \\
\subfloat{\includegraphics[width=0.7\textwidth]{figs/analysis/meanPredQcdDivEwk}}
 \\
...
%QCD: An adequate level of agreement is found between the three independent 
%estimates.
...
%maybe add stat unc (1-50\%?), qcd syst, mht syst (anything else?)
\begin{table}[t]
\centering
\begin{tabular}{ccc}
\hline
...
\begin{figure}[h!]
\subfloat{\includegraphics[width=0.5\textwidth]{figs/analysis/transferfactors/tfratio_mu_Ttw_2d_puWeightUp}}~
\subfloat{\includegraphics[width=0.5\textwidth]{figs/analysis/transferfactors/tfratio_mu_Ttw_2d_puWeightDown}}\\
\subfloat{\includegraphics[width=0.5\textwidth]{figs/analysis/transferfactors/tfratio_mumu_Zinv_2d_puWeightUp}}~
\subfloat{\includegraphics[width=0.5\textwidth]{figs/analysis/transferfactors/tfratio_mumu_Zinv_2d_puWeightDown}}\\
...
The effect on the transfer factors is usually small, at the level of a few 
...
%small because cancel out . Largely cancel out in the TF ratio.
...
%Adam/AN: As the HT and jet multiplicity binning is mirrored in signal and 
%control regions, the effect of jet energy scale on the TFs is expected to be 
%small. However, the jet energy scale can still have an effect due to jets 
%moving in and out of acceptance (above and below 40 GeV
...
%\textbf{[todo: how/what does this mean]}, 
...
\znnj process.
% note bit different for trigger - only affects CR yield not SR

%veto anticorrelated between sr and cr
%The scale factors are near-unity and their 1245 uncertainties are at the 
%%%percent level.
...
%For instance, a correction factor of 1.1 
%(or similarly, 0.9), is assigned a 10\% uncertainty. 
...
%. Theuncertainties in the Hmiss1206 T dimension can be as large as 30%.

% NOTE: NNLO xs kfactors not used (neither when applying NLO nor when -1sigma)
%https://github.com/CMSRA1/AlphaTools/commit/b0f7985b724fa839be92bcb84d0276ee82b7703d
...
%Fig. 87 (App. E) show the effect on the transfer factors as a function of the 
%various 1215 discriminating variables. Uncertainties are in typically at the 
%%%percent level
...
% \textbf{[TODO: W 1.08, tt 1.04 - reference?]}. 
The changes in the transfer factors are then assigned as systematic 
uncertainties on their values.
%https://github.com/CMSRA1/AlphaTools/blob/v1.10.x/analysis/Producers/WeightXsVariations.py

The variations on the \Tmutottw transfer factors is shown in 
...
%not subsubsec?
%maybe just put in qcd estimation sec.
%previous sections are for EWK background.
%formula syst?
...
% due to extrapolations made in the background estimation and 
...
%To avoid double counting the uncertainties due to the simulation correction 
...
Linear trends as a function of \scalht and \njet are observed in certain 
...
factors.
%because TFs account for differences in acceptance, branching ratio etc.
...
%To avoid double counting the \alphat and \bdphi 
%uncertainties as the variables are somewhat correlated, the systematic 
%uncertainties are taken from the \alphat closure test for the 
%$\scalht<900$~GeV bins and from the \bdphi closure test for the 
%$\scalht>900$~GeV bins.
% we don't actually do this - see nuisance plots (note more entries for bdphi 
%because no alphat cuts/CTs for ht>900), also note these are fine ht bins, also 
%note we have CTs to account for merged bins eg 400-inf and 600-inf

\subsubsection{Extrapolation in the W$^+$/W$^-$ proportion}
% leptons/neutrinos must be LH, anti-leptons/anti-neutrinos RH
% wiki: Only left-handed fermions participate in the weak interaction
% lol Adam and AN are wrong
...
%alphat/bdphi syst applied to both TFs, polarisation only relevant for mu2ttw 
%TF.

%\subsubsection{SITV?}
%fixme: Probably leave out unless you have time to understand it.

\begin{figure}[t]
\subfloat{\includegraphics[width=0.5\textwidth]{figs/analysis/closuretests/AlphaT/SingleMu_alphaTExtrapolation_ht}}~~
\subfloat{\includegraphics[width=0.5\textwidth]{figs/analysis/closuretests/AlphaT/SingleMu_alphaTExtrapolation_nJet}}\\
\subfloat{\includegraphics[width=0.5\textwidth]{figs/analysis/closuretests/bDPhi/SingleMu_bdphiExtrapolation_ht}}~~
\subfloat{\includegraphics[width=0.5\textwidth]{figs/analysis/closuretests/bDPhi/SingleMu_bdphiExtrapolation_nJet}}\\
\subfloat{\includegraphics[width=0.5\textwidth]{figs/analysis/closuretests/WPol/SingleMuPlus_to_SingleMuMinus_ht}}~~
\subfloat{\includegraphics[width=0.5\textwidth]{figs/analysis/closuretests/WPol/SingleMuPlus_to_SingleMuMinus_nJet}}\\
...
%The \mht distribution obtained from simulation will be referred to as the 
%nominal \mht distribution or shape. 
...
%also consider MC variations - go over effects on mht shape??.. (see NLO sec in 
%AN) (no cancellation as with TFs)
